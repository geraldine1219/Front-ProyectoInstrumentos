...
type Alumno = {
    ...
    dni: string;
    edad: number;
    fechaNacimientoProfesor: string;
...
export default function Admin() {
    ...
    const [profesores, setprofesores] = useState<Alumno[]>([]);
    ...
    useEffect(() => {
        axios.get(`${apiUrl}/api/profesor/list`)
            .then(res => {
                setprofesores(res.data.user);
            })
            .catch(err => {
    ...
    return (
        <>
            <div style={{ display: "flex" }}>
                <Sidebar collapsed={collapsed} setCollapsed={setCollapsed} selectedIconPage="Profesor" selectedSidebarNav="Administrador"/>
                <div className="content-sidebear" style={{ width: collapsed ? "90vw" : "80vw" }}>
                    <h1 className="titulo-principal">Profesor</h1>
                    <hr className="linea-azul"></hr>

                    <h2 onClick={() => setShowRegister(!showRegister)} className="titulo-principal">
                        Registrar Profesor {showRegister ? <ChevronDown size={25} /> : <ChevronRight size={25} />}
                    </h2>
                    ...
                    {showRegister && (<>
                        <form onSubmit={handleSubmitRegister} className="space-y-5">
                            ...
                            <div className="space-y-5 form-grid-container-data">
                                ...
                                <div className="form-group-register">
                                    <label htmlFor="fechaNacimientoProfesor" className="form-label">Fecha de Nacimiento</label>
                                    <input
                                        type="date"
                                        name="fechaNacimientoProfesor"
                                        value={formRegister.fechaNacimientoProfesor}
                                        onChange={handleChangeRegister}
                                        required
                                        className="input-field"
                    ...
                    <h2 onClick={() => setShowTable(!showTable)}  className="titulo-principal">
                        Profesores Registrados {showTable ? <ChevronDown size={25} /> : <ChevronRight size={25} />}
                    </h2>
                                    ...
                                    <tbody>
                                        {profesores.map((alumno, index) => (
                                            <tr key={index} className="text-center">
                                                ...
                                                <td className="px-4 py-2 border">{alumno.fechaNacimientoProfesor}</td>
